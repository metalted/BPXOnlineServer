...
{
    Task<Result<AuthenticationData>> Login(LoginResource resource);
...
public class AuthenticationService : IAuthenticationService
{
    ...
    public async Task<Result<AuthenticationData>> Login(LoginResource resource)
    {
        Result<string> authenticationResult = await _steamService.Authenticate(resource.AuthenticationTicket);
        if (authenticationResult.IsFailed)
        {
            return authenticationResult.ToResult();
        }
        
        if (authenticationResult.Value != resource.SteamId.ToString())
        {
            return Result.Fail("Steam ID does not match");
        }

        if (!_userService.TryGet(resource.SteamId, out User? user))
        {
            user = _userService.Create(resource.SteamId, resource.SteamName);
        }

        string accessToken = _jwtService.GenerateToken(resource.SteamId);
